...
// Generate mock result for fallback scenarios
export interface MockOfferResult {
  provider: string;
  plan_name: string;
  price_kwh?: number;
  unit_price_eur_smc?: number;
  fixed_fee_eur_mo: number;
  simulated_cost: number;
  risparmio_vs_attuale_eur: number;
  commodity: string;
  is_mock: boolean;
}

export const generateMockResult = (tipo: 'luce' | 'gas'): MockOfferResult => {
  if (tipo === 'gas') {
    return {
      provider: "Esempio Energia",
      plan_name: "Click Gas Base",
      unit_price_eur_smc: 0.85,
      fixed_fee_eur_mo: 8.50,
      simulated_cost: 1120,
      risparmio_vs_attuale_eur: 45,
      commodity: 'gas',
      is_mock: true
    };
  }
  ...
  return {
    provider: "Esempio Energia",
    plan_name: "Click Luce Base",
    price_kwh: 0.25,
    fixed_fee_eur_mo: 6.90,
    simulated_cost: 830,
    risparmio_vs_attuale_eur: 45,
    commodity: 'luce',
    is_mock: true
