...
function parseAzureResult(result: any): any {
  ...
  console.log("[DIAGNOSTIC] Content preview (first 1000 chars):", content.substring(0, 1000));
  console.log("[DIAGNOSTIC] Searching for fasce keywords...");
  const fasceMatches = content.match(/.{0,80}(F1|F2|F3|fascia).{0,80}/gi);
  console.log("[DIAGNOSTIC] Fasce matches found:", fasceMatches ? fasceMatches.length : 0);
  if (fasceMatches) console.log("[DIAGNOSTIC] Sample matches:", fasceMatches.slice(0, 3));
...
serve(async (req) => {
  ...
  try {
    ...
    const { error: dbError } = await supabase.from("ocr_results").insert({
      ...
      gas_smc: annualSmc,
      // FIX: Add fasce orarie data from extracted bolletta_luce
      f1_kwh: extractedData.bolletta_luce?.consumi_fasce?.f1 || null,
      f2_kwh: extractedData.bolletta_luce?.consumi_fasce?.f2 || null,
