...
async function scrapeWithFirecrawl(url: string, provider: string, planName: string, fixedFeeYear: number = 0, commodity: string = 'gas', batchId: number = 3) {
  try {
    console.log(`[BATCH-3] Scraping ${provider} from ${url}`);
    
    const firecrawlApiKey = Deno.env.get('FIRECRAWL_API_KEY');
    if (!firecrawlApiKey) {
      console.error('[BATCH-3] FIRECRAWL_API_KEY not found');
    ...
    if (!scrapeResponse.ok) {
      ...
      console.error(`[BATCH-3] Firecrawl error for ${provider}: ${scrapeResponse.status}`);
    ...
    if (!scrapeResult.success || !scrapeResult.data) {
      console.error(`[BATCH-3] Scraping failed for ${provider}`);
      return { error: 'Scraping failed', provider, batch_id: batchId };
    }

    const textContent = scrapeResult.data.markdown || '';
    console.log(`[BATCH-3] ${provider} content: ${textContent.length} chars`);
    ...
    console.log(`[BATCH-3] ${provider}: price = ${price}`);
    ...
    console.error(`[BATCH-3] Error scraping ${provider}:`, error);
...
serve(async (req) => {
  ...
  try {
    console.log('[BATCH-3] Starting batch 3 scraping (providers 9-12, gas)...');
    ...
    console.log(`[BATCH-3] Completed: ${successCount}/${results.length} successful`);
    ...
    console.error('[BATCH-3] Error:', error);
    return new Response(JSON.stringify({ 
